defmodule TR do
  ...
  def list_groups(trmFolders) do
    runlog("...TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEESSSSSSSSSST...")
    
    trmFolders
    |> Enum.map( fn(e) ->
          Path.join(~w(s: _slatedb u #{e}))
       end)
    |> Enum.map(
         fn(path) -> 
...
TR.list_groups(~w(P3)) |> TR.iterate_groups